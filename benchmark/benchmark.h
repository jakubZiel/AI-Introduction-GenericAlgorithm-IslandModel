...
void record_errors(double *errors, Islands_Evolution &island_model, int elitism, int max_generations, int num_of_migrations, int func_num);
...
void generate_data_island_model(int max_generations, double cross_probability, double mutation_power, int genome_length,
    ...
    while (run_counter < runs){
        ...
        record_errors(errors + run_counter * 14, island_model, elitism, max_generations, num_of_islands, func_num);
        run_counter++;
    }
...
void record_errors(double *errors, Islands_Evolution &island_model, int elitism, int max_generations, int num_of_migrations, int func_num){
    /*island_model.islands_evolution_run(elitism, int(max_generations * 0.01), num_of_migrations, func_num);
    errors[0] = island_model.get_best_from_archipelago();

    island_model.run(elitism, int(max_generations * 0.01), func_num);
    errors[1] = island_model.population.population[0].fitness;

    island_model.run(elitism, int(max_generations * 0.01), func_num);
    errors[2] = island_model.population.population[0].fitness;

    island_model.run(elitism, int(max_generations * 0.02), func_num);
    errors[3] = island_model.population.population[0].fitness;

    island_model.run(elitism, int(max_generations * 0.05), func_num);
    errors[4] = island_model.population.population[0].fitness;

    island_model.run(elitism, int(max_generations * 0.1), func_num);
    errors[5] = island_model.population.population[0].fitness;

    island_model.run(elitism, int(max_generations * 0.1), func_num);
    errors[6] = island_model.population.population[0].fitness;

    island_model.run(elitism, int(max_generations * 0.1), func_num);
    errors[7] = island_model.population.population[0].fitness;

    island_model.run(elitism, int(max_generations * 0.1), func_num);
    errors[8] = island_model.population.population[0].fitness;

    island_model.run(elitism, int(max_generations * 0.1), func_num);
    errors[9] = island_model.population.population[0].fitness;

    island_model.run(elitism, int(max_generations * 0.1), func_num);
    errors[10] = island_model.population.population[0].fitness;

    island_model.run(elitism, int(max_generations * 0.1), func_num);
    errors[11] = island_model.population.population[0].fitness;

    island_model.run(elitism, int(max_generations * 0.1), func_num);
    errors[12] = island_model.population.population[0].fitness;

    island_model.run(elitism, int(max_generations * 0.1), func_num);
    errors[13] = island_model.population.population[0].fitness;
    */
}
